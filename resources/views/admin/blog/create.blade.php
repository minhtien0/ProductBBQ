@extends('admin.index')
@section('content')
    <form action="{{ route('admin.blog.store') }}" method="POST" enctype="multipart/form-data" class="flex w-full">
        @csrf
        <div class="flex-1 bg-white rounded-lg border border-gray-200 shadow-sm p-6">
            <h1 class="text-lg font-bold text-gray-800 mb-6">Thêm Blog Mới</h1>
            ...
            <div class="mb-6">
                <label for="title" class="block text-xs font-medium text-gray-700 mb-1">Tiêu đề <span
                        class="text-red-600">*</span></label>
                <input type="text" id="title" name="title" value="{{ old('title') }}"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500" required>
            </div>
            <div class="mb-6">
                <label for="type" class="block text-xs font-medium text-gray-700 mb-1">Loại Blog <span
                        class="text-red-600">*</span></label>
            ...
            <div class="mb-6">
                <label for="content" class="block text-xs font-medium text-gray-700 mb-1">Nội dung <span
                        class="text-red-600">*</span></label>
                <textarea id="content" name="content" rows="10"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500">{{ old('content') }}</textarea>
            </div>
        </div>
