...
@section('content')
    ...
    <body class="bg-gradient-to-br from-slate-50 to-blue-50 min-h-screen">
        ...
        <div class="container mx-auto p-6" x-data="bookingDetailManager()">
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
                    ...
                    <div class="glass-effect rounded-xl p-6 slide-in-up">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="flex items-start gap-4">
                                <div
                                    class="w-16 h-16 bg-gradient-to-br from-blue-400 to-blue-600 rounded-full flex items-center justify-center text-white text-2xl font-bold shadow-lg">
                                    <span>User</span>
                                </div>
                                <div class="flex-1">
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                    ...
                                    <span class="font-semibold text-gray-900">{{ $details->quantitypeople }}
                                        người</span>
                                </div>
                                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                    <span class="text-gray-600 flex items-center gap-2">
                                        ...
                                        {{  $details->quantity_table   }} người)
                                    </span>
                                    <span class="font-semibold text-gray-900">
                                        <select name="table_id" class="ml-2 px-2 py-1 border rounded">
                                            <option value="">Chọn</option>
                                            @foreach($listTable as $table)
                                                <option value="{{ $table->id }}" {{ isset($details->table_id) && $details->table_id == $table->id ? 'selected' : '' }}>
                                                    {{ $table->number }} ({{ $table->quantity }} người)
                                                </option>
                                            @endforeach
                                        </select>
                                    </span>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-6">
                                ...
                                <div class="info-card bg-white p-6 rounded-xl shadow-sm">
                                    ...
                                    <p class="text-lg font-bold text-gray-900">
                                        {{ \Carbon\Carbon::parse($details->time_booking)->format('d/m/Y H:i') }}
                                    </p>
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="info-card bg-white p-6 rounded-xl shadow-sm">
                                    ...
                                    <p class="text-lg font-bold text-gray-900">
                                        {{ \Carbon\Carbon::parse($details->time_order)->format('d/m/Y H:i') }}
                                    </p>
                        ...
                        <div class="space-y-4">
                            <div class="priority-medium p-4 rounded-lg">
                                ...
                                <textarea name="notes"
                                    ...
                                    rows="4">{{ $details->notes ?? 'Chưa có ghi chú' }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="glass-effect rounded-xl p-6 slide-in-up" style="animation-delay: 0.5s">
                        ...
                        <div class="space-y-3" id="booking-actions">
                            @if($details->status == 'Chờ xác nhận')
                            ...
                            @endif
                            <!-- Form Hủy -->
                            @if($details->status !== 'Đã hủy')
                                <form action="{{ route('admin.booking.cancel', $details->id_booking) }}" method="POST"
                                    onsubmit="return confirm('Bạn chắc chắn muốn hủy đặt bàn này?');">
                                    @csrf
                            ...
                            @else
                                <button
                                    class="w-full bg-gray-300 text-gray-700 py-3 rounded-lg font-medium cursor-not-allowed flex items-center justify-center gap-2"
                                    disabled>
            ...
            <div x-show="notification.show" x-transition:enter="transition ease-out duration-300"
                ...
                x-transition:leave-end="opacity-0 transform translate-y-2" class="fixed bottom-6 left-6 z-50 max-w-sm">
    ...
    <script>
        $(document).ready(function () {
            // Gọi điện thoại
            $('#callCustomerBtn').click(function () {
                var phone = '{{ $details->sdt }}';
                if (phone) {
                    window.location.href = 'tel:' + phone;
                    $('select[name="table_id"]').val('{{ $details->table_id }}');
                } else {
            ...
            $('#sendSMSBtn').click(function () {
                var phone = '{{ $details->sdt }}';
                var smsBody = encodeURIComponent('Xin chào {{ $details->nameuser }}, nhà hàng xin xác nhận về đơn đặt bàn của bạn.');
                if (phone) {
            ...
            $('#sendEmailBtn').click(function () {
                if (!confirm('Bạn chắc chắn muốn gửi email cho khách hàng?')) return;
                $.ajax({
                    url: '{{ route("admin.booking.sendEmail", $details->id_booking) }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
                    ...
                    error: function (xhr) {
                        let msg = xhr.responseJSON?.message || 'Gửi email thất bại!';
                        alert(msg);
                        $('select[name="table_id"]').val('{{ $details->table_id }}');
                    }
                });
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#confirmBookingBtn').click(function () {
                ...
                $.ajax({
                    url: '{{ route("admin.booking.confirm", $details->id_booking) }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        table_id: tableId
            ...
            // Đổi bàn
            $('#changeTableBtn').click(function (e) {
                e.preventDefault();
                // LẤY GIÁ TRỊ ĐÚNG từ select đã có
                var tableId = $('select[name="table_id"]').val();
                var notes = $('textarea[name="notes"]').val();
                if (!tableId) {
                    alert('Bạn phải chọn bàn mới!');
                    return;
                }
                $.ajax({
                    url: '{{ route("admin.booking.changeTable", $details->id_booking) }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        table_id: tableId,
            ...
            $('#cancelBookingBtn').click(function (e) {
                ...
                $.ajax({
                    url: '{{ route("admin.booking.cancel", $details->id_booking) }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
                    success: function (res) {
                        showMsg(res.message || 'Hủy đặt bàn thành công!', 'success');
            ...
            function showMsg(msg, type) {
                $('#bookingActionMsg').html(`<div class="rounded px-4 py-3 ${type == 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">${msg}</div>`);
            }
        });
    </script>
