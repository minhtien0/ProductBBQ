                ...
                <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                    ...
                    <a href="{{ route('admin.order.bringback') }}"
                        ...
                        <span class="flex items-center">
                            <span class="inline-flex items-center justify-center w-5 h-5 mr-2 text-xs font-bold bg-gray-100 rounded-full {{ is_null($statusorder) ? 'bg-orange-100 text-orange-600' : 'text-gray-600' }}">
                                {{ $counts['all'] }}
                            </span>
                            Tất Cả
                        </span>
                    ...
                    @foreach($statuses as $s)
                        <a href="{{ route('admin.order.bringback', ['statusorder' => $s]) }}"
                            ...
                            <span class="flex items-center">
                                <span class="inline-flex items-center justify-center w-5 h-5 mr-2 text-xs font-bold bg-gray-100 rounded-full {{ $statusorder === $s ? 'bg-orange-100 text-orange-600' : 'text-gray-600' }}">
                                    {{ $counts[$s] }}
                                </span>
                                {{ $s }}
                                @if($s === 'Chờ xác nhận')
        ...
        <div class="mb-6 p-4 bg-gray-50 rounded-xl">
            <div class="flex flex-wrap items-center gap-4">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <input type="date" class="px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        <label class="flex items-center">
                            <input type="checkbox" id="selectAll" class="w-4 h-4 text-orange-600 border-gray-300 rounded focus:ring-orange-500">
                    ...
                    <div class="flex items-center space-x-2">
                        <select class="px-3 py-1 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-orange-500">
                            <option>Thao tác hàng loạt</option>
                            <option>Xuất đã chọn</option>
                            <option>In đã chọn</option>
                        </select>
                        <button class="px-3 py-1 text-sm bg-orange-500 text-white rounded-lg hover:bg-orange-600">
                            Thực hiện
                        </button>
                    </div>
                </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @php
                                        $statusColors = [
                                            ...
                                            'Đã giao' => 'bg-gray-100 text-gray-800',
        ...
        @if($orders->hasPages())
            <div class="mt-6 flex items-center justify-between">
                <div class="text-sm text-gray-700">
                    Hiển thị {{ $orders->firstItem() }} đến {{ $orders->lastItem() }} trong tổng số {{ $orders->total() }} kết quả
                </div>
